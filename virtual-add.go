...
func createAddFlags(cmd *cobra.Command, flags *VirtualAddFlags) {
	...
	cmd.Flags().StringVar(&flags.transparent, "transparent", "N", "When using Layer 7, when this is enabled the connection arriving at the Real Server appears to come directly from the client. Alternatively, the connection can be non-transparent which means that the connections at the Real Server appear to come from the LoadMaster.")
	...
	cmd.Flags().StringVar(&flags.sslAcceleration, "ssl-acceleration", "", "Enable SSL handling on this Virtual Service (activate automatically if port 443 is used).")
}

func virtualAddRun(cmd *cobra.Command, args []string) {
	if virtualAddFlags.port == "443" && virtualAddFlags.sslAcceleration == "" {
		virtualAddFlags.sslAcceleration = "Y"
	}
